...
//        commandRegistry.loadBuildInCommandCollections();
    ...
    @Test
    public void test() {
    	IEBusCommand command = commandRegistry.getCommandById("vrc700_general", "gen.continous_heating");
    	IEBusCommandMethod method = command.getCommandMethod(Method.GET);
    	
    	for (IEBusValue value : method.getMasterTypes()) {
			logger.info(value.toString());
		}
    	
    	for (IEBusValue value : method.getSlaveTypes()) {
			logger.info(value.toString());
		}
    }
//
